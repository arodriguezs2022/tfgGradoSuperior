...
public interface HabitacionRepository extends JpaRepository<Habitacion, Integer>{
	@Query(value = "SELECT h FROM Habitaciones h WHERE NOT EXISTS (SELECT NULL FROM Reserva r WHERE r.habitacion = h.id AND :fecha2 <= r.fecha_salida AND :fecha1 >= r.fecha_entrada)")
